...
Approach:
The brute force solution is to use a recursive approach to explore all possible ways to reach the target amount n using the given denominations. At each step, the algorithm considers either including a coin in the current combination or skipping it.
...
Approach:
The optimized solution uses dynamic programming to build up the number of ways to make change for all amounts from 0 to n. The idea is to use a ways array where ways[i] represents the number of ways to make change for amount i. We iterate through each denomination and update the ways array.

Time Complexity:
O(n * m): Where n is the target amount and m is the number of denominations. This is because we are iterating through each denomination for each amount up to n.
Space Complexity:
